...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('retourventes.update',$venteretour->id) }}" method="POST" id="editretourvente">
                @method('PUT')
                @csrf
                @if ($errors->any())
                    ...
                    <div class="col-lg-3 col-sm-6 col-12">
                        <div class="form-group">
                            ...
                            <div class="input-groupicon">
                                <input type="text" class="form-control" value="{{$venteretour->vente->num_vente}}"  id="referencevente_vente" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-6 col-12">
                        <div class="form-group">
                            <label>Ref. Retour Vente </label>
                            <div class="input-groupicon">
                                <input type="text" class="form-control" value="{{$venteretour->num_retourvente}}"  id="referenceretourvente_vente" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-6 col-12">
                        <div class="form-group">
                            <label>Date Retour Vente</label>
                            <div class="input-groupicon">
                                <input type="date" class="form-control" value="{{$venteretour->date_retourvente}}" name="date_retourvente"   id="retourventedate_vente">
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="productimgname">
                                        <input type="text" value="{{$ligneventeretour->produit->nom_produit}}" class="form-control-plaintext border-0" readonly/>
                                        <input type="hidden" value="{{$ligneventeretour->produit_id}}" name="produit_id[]" />
                                      </td>
                                      <td> <input type="number" name="quantite_ligneretourvente[]" value="{{$ligneventeretour->quantite_ligneretourvente}}" class="form-control quantite_ligneretourvente" /></td>
                                      <td><input type="text" name="prixvente_ligneretourvente[]" value="{{$ligneventeretour->prixvente_ligneretourvente}}"  class="form-control-plaintext border-0 prixvente_ligneretourvente" readonly/></td>
                                      <td><input type="text" name="soustotal_ligneretourvente[]" value="{{$ligneventeretour->soustotal_ligneretourvente}}" class="form-control-plaintext border-0 soustotal_ligneretourvente" readonly/></td>
                                      <td>
                    ...
                    <div class="col-lg-12 float-md-right">
                        <div class="total-order">
                            <ul>
                                <li class="total">
                                    <h4>Grand Total</h4>
                                    <h5><input type="text" name="total_retourvente" value="{{$venteretour->total_retourvente}}" id="total_retour_achat"
                                            class="form-control-plaintext border-0 total_retourvente"  readonly /></h5>
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="form-group">
                            ...
                            <textarea class="form-control description_retourvente" name="description_retourvente">{{$venteretour->description_retourvente}}</textarea>
                        </div>
                    </div>
                    <div class="col-lg-12">
                        <button class="btn btn-submit me-2">Submit</button>
                        <a href="{{ route('retourventes.index') }}" class="btn btn-cancel">Cancel</a>
                    </div>
                </div>
            </form>
        </div>
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            $("#editretourvente").validate({
                rules: {
                    ...
                    "quantite_ligneretourvente[]": {
                        required: true,
                        digits: true,

                    },

                    "prixvente_ligneretourvente[]": {
                        required: true,
                        digits: true
                    },
                    "soustotal_ligneretourvente[]": {
                        required: true,
                        digits: true
                    },
                    total_retourvente: {
                ...
                messages: {
                    date_retourvente: {
                        required: "Date Return Sale is required",
                        date: "Date Return Sale is must date"
                    },
        ...
        function totalVente() {
            var totalVente = 0;
            $('.soustotal_ligneretourvente').each(function(i, e) {
                var soustotal = $(this).val() - 0;
                totalVente += soustotal;
            })
            $('.total_retourvente').val(totalVente);
        }
        //Changer quantite
        $("table").on('input', '.quantite_ligneretourvente', function() {
            rowindex = $(this).closest('tr').index();
            if ($(this).val() < 1 && $(this).val() != '') {
                $('table tbody tr:nth-child(' + (rowindex + 1) + ') .quantite_ligneretourvente').val(1);
        ...
        //     checkQuantity($(this).val());
        // });
        $('tbody').delegate('.quantite_ligneretourvente', 'change', function() {
            var tr = $(this).parent().parent();
            var qty = tr.find('.quantite_ligneretourvente').val() - 0;
            var prixvente = tr.find('.prixvente_ligneretourvente').val() - 0;
            var soustotal = qty * prixvente;
            tr.find('.soustotal_ligneretourvente').val(soustotal);
            totalVente();
